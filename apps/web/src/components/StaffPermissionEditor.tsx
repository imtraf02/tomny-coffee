...
type StaffUser = { id: string; email: string; displayName: string; active: boolean; permissions: string[] }
...
export function StaffPermissionEditor({
  ...
  const toggleGroup = (groupPermissions: Permission[], checkAll: boolean) => {
    ...
    if (checkAll) {
      setSelected((curr) => [...new Set([...curr, ...codes])])
    } else {
      setSelected((curr) => curr.filter((c) => !codes.includes(c)))
    }
  }

  return (
    <>
      <Button variant="secondary" size="sm" disabled={!canManage} onClick={openEditor} className="action-edit-btn flex items-center gap-1">
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Viewport className="dialog-viewport">
            <Dialog.Popup className="product-mockup-dialog" style={{ maxWidth: '580px' }}>
              <div className="product-mockup-form">
                <div className="flex items-start justify-between pb-4 border-b border-[#ede6de]">
                  <div>
                    ...
                    <Dialog.Description className="text-xs text-[#8c8177] mt-1">
                      {user.email} · Gán quyền chi tiết theo từng phân hệ vận hành.
                    </Dialog.Description>
                  </div>
                ...
                <div className="p-3 my-3 bg-[#fffdfa] border border-[#ede6de] rounded-xl flex items-center justify-between">
                  <label className="flex items-center gap-2.5 cursor-pointer text-xs font-semibold select-none">
                    <Checkbox
                      checked={active}
                      onCheckedChange={(checked) => setActive(checked === true)}
                    />
                    <span>{active ? 'Tài khoản đang được kích hoạt hoạt động' : 'Tài khoản đang bị vô hiệu hóa (khóa đăng nhập)'}</span>
                  </label>
                  <span className={active ? 'text-xs text-[var(--moss)] font-bold' : 'text-xs text-[var(--ember)] font-bold'}>
                    {active ? '● Hoạt động' : '● Đã khóa'}
                  </span>
                </div>

                {/* Categorized Permissions Accordion/Fieldsets */}
                <div className="grid gap-3 max-h-96 overflow-y-auto pr-1">
                  {groups.map(([group, items]) => {
                    const allChecked = items.every((i) => selected.includes(i.code))
                    return (
                      <div key={group} className="border border-[#e5ddd6] rounded-xl p-3.5 bg-white">
                          ...
                          {items.map((permission) => (
                            ...
                            >
                              ...
                              <div className="min-w-0">
                                ...
                                <small className="block text-[10px] text-[#8c8177] font-data truncate">{permission.code}</small>
                              </div>
                            </label>
                          ))}
                        </div>
                ...
                {/* Footer Actions */}
                <div className="product-mockup-footer mt-4">
