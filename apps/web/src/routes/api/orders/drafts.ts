...
import { addLine, cancelDraft, createDraft, linkTable, listDrafts, mergeDrafts, moveDraft, splitDraft, unlinkTable, updateNote, voidLine } from '../../../server/order-service'
...
const createSchema = z.object({ action: z.literal('create'), source: z.enum(['table', 'counter', 'takeaway']).default('table'), tableId: entityId.optional(), tableIds: z.array(entityId).min(1).max(8).optional(), note: z.string().max(500).default(''), lines: z.array(selection).max(20).default([]), idempotencyKey: z.string().uuid() })
...
const linkTableSchema = z.object({ action: z.literal('linkTable'), orderId: z.string().uuid(), expectedVersion: z.number().int().positive(), tableId: entityId })
const unlinkTableSchema = z.object({ action: z.literal('unlinkTable'), orderId: z.string().uuid(), expectedVersion: z.number().int().positive(), tableId: entityId })
const inputSchema = z.discriminatedUnion('action', [createSchema, addSchema, voidSchema, noteSchema, moveSchema, cancelSchema, splitSchema, mergeSchema, linkTableSchema, unlinkTableSchema])
...
async function mutateDrafts({ request }: { request: Request }) {
  ...
  try {
    switch (input.action) {
      ...
      case 'linkTable':
        return Response.json(await linkTable(db, actor, input))
      case 'unlinkTable':
        return Response.json(await unlinkTable(db, actor, input))
    }
  } catch (error) {
