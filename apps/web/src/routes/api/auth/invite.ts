...
async function acceptInvite({ request }: { request: Request }) {
  ...
  const now = Date.now()
  const username = (invite.email.includes('@') ? invite.email.split('@')[0] : invite.email).toLowerCase()
  ...
  const statements: D1PreparedStatement[] = [
    env.DB.prepare('INSERT INTO users (id, username, email, display_name, password_hash, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, ?)').bind(userId, username, invite.email, invite.displayName || username, await hashPassword(input.data.password), now, now),
  ...
  statements.push(env.DB.prepare('INSERT INTO audit_logs (id, actor_id, entity_type, entity_id, action, detail_json, created_at) VALUES (?, NULL, ?, ?, ?, ?, ?)').bind(crypto.randomUUID(), 'user', userId, 'created_from_invite', JSON.stringify({ inviteId: invite.id, username }), now))
  await env.DB.batch(statements)
  const session = await createSession(userId)
  return Response.json({ user: { id: userId, username, displayName: invite.displayName } }, { headers: { 'set-cookie': sessionCookie(session.token, session.expiresAt, request) } })
}
